...
const AnimatedCircle = ({ children }) => {
  ...
  return (
    <div className="relative inline-block ">
      <button
        onClick={handleClick}
        className="relative z-10 px-2 py-1 text-xl font-medium text-white transition-colors"
      >
        {children}
      </button>
      <AnimatePresence>
      {isAnimating && (
        <motion.svg
          className="absolute -top-4 -left-4 w-[calc(100%+2rem)] h-[calc(100%+2rem)] pointer-events-nones overflow-visible"
          width="112"
          height="62"
          viewBox="0 0 112 62"
          fill="none"
